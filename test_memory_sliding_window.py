...
def measure_attention_memory(attention_type='standard', batch_size=4, seq_len=1024):
    ...
    if attention_type == 'sliding_window':
        config = GPTConfig(
            vocab_size=50257, n_layer=12, n_head=12, n_embd=768//2,
            attention_type='sliding_window', window_size=64,
            post_norm=True, qk_norm=True
        )
    else:
        config = GPTConfig(
            vocab_size=50257, n_layer=6, n_head=6, n_embd=36,
...
def compare_memory():
    ...
    configs = [
        # ('standard', 12, 512),
        # ('sliding_window', 12, 512),
        # ('standard', 12, 1024),
        # ('sliding_window', 12, 1024),
        ('standard', 12, 2048),
        # ('sliding_window', 12, 2048),
    ]
